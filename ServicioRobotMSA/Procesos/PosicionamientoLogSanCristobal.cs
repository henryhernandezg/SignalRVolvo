...
{
    public class PosicionamientoLogSanCristobal : IProcesoMasivo
    {
        public void EjecutarProceso()
        {
            try
            {
                ...
                carpetaRobot = ConfigurationManager.AppSettings["directorioArchivosRobot"] + "TablaPosicionesLog\\Zonal\\SanCristobal\\" + stringFechaCarpeta;
                ...
                do
                {
                    ...
                    String comando = @" /testcontainer:" + Comillas + @"C:\RobotWeb\RobotWeb\RobotWeb\RobotPosicionamientoLogCEXPZonalSanCristobal.loadtest" + Comillas + @" /resultsfile:C:\ArchivosRobot\Ejecuciones\ResultsRobotPosicionamientoLogCEXPZonalSanCristobal" + stringFechaCarpeta + numeroIntentos.ToString() + @".trx /testsettings:" + Comillas + @"C:\RobotWeb\RobotWeb\Local.testsettings" + Comillas + " /usestderr";
                    if (fCount == 651 || numeroIntentos == 0)
                    {
                        Zonal = false;
                    }
                    else
                ...
                carpetaRobot = ConfigurationManager.AppSettings["directorioArchivosRobot"] + "TablaPosicionesLog\\Troncal\\SanCristobal\\" + stringFechaCarpeta;
                do
                {
                    if (!Directory.Exists(carpetaRobot))
                    {
                    ...
                    // searches the current directory
                    int fCount = Directory.GetFiles(carpetaRobot, "*", SearchOption.TopDirectoryOnly).Length;
                    String comando = @" /testcontainer:" + Comillas + @"C:\RobotWeb\RobotWeb\RobotWeb\RobotPosicionamientoLogCEXPTroncalSanCristobal.loadtest" + Comillas + @" /resultsfile:C:\ArchivosRobot\Ejecuciones\ResultsRobotPosicionamientoLogCEXPTroncalSanCristobal" + stringFechaCarpeta + numeroIntentos.ToString() + @".trx /testsettings:" + Comillas + @"C:\RobotWeb\RobotWeb\Local.testsettings" + Comillas + " /usestderr";
                    if (fCount == 450 || numeroIntentos == 0)
                    {
                        Troncal = false;
                    }
                    else
